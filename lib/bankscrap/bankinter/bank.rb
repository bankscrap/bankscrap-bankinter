...
module Bankscrap
  module Bankinter
    class Bank < ::Bankscrap::Bank
      BASE_ENDPOINT = 'https://www.bankinter.com'.freeze
      LOGIN_ENDPOINT = '/www/es-es/cgi/ebk+login'.freeze
      TRANSACTIONS_ENDPOINT = '/www/es-es/cgi/ebk+opr+buscadormov'.freeze
      ...
      def build_transaction(data, account)
        ...
        if data['accountBalanceAfterMovement']
          balance = Money.new(data['accountBalanceAfterMovement'] * 100, data['currency'])
